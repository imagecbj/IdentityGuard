...
def show_cross_attention(prompt: str,
    ...
    # 把同一张图片的5个token图横向拼接起来
    concat_vis = np.concatenate(images, axis=1)  # axis=1表示横向拼接

    # 保存拼接后的大图
    filename = f"att_map/img_concat11.png"
    filepath = os.path.join(filename)
    cv2.imwrite(filepath, concat_vis)
...
def show_image_relevance(image_relevance, image: Image.Image, relevnace_res=16):
    ...
    # image_relevance1 = torch.nn.functional.interpolate(image_relevance.unsqueeze(0).unsqueeze(0), size=8, mode='bilinear')
    # print(image_relevance1)
